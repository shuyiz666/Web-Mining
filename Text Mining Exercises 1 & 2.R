...
email.texts <- c("Jon_Smith@bu.edu",
                 ...
                 "Here is the address of the lecturer: ", "123 Main St. #25","Boston MA 02155") 
# Step 1: Create corpus
corpus.txt <- VCorpus(VectorSource(data.frame(email.texts)))
# email.df <- data.frame(doc_id=row.names(1:length(email.texts)),text=email.texts)
# corpus.txt <- Corpus(DataframeSource(email.df))
...
plot(cl,main="Jaccard Distance",xlab="Term Clustering Dendrogram using",sub = "...") # Set plot title
...
plot(cls,main="Jaccard Distance with Stemming",xlab="Term Clustering Dendrogram using",sub = "...") # Set plot title
...
plot(cl2,main="Optimal String Alignment",xlab="Term Clustering Dendrogram using",sub = "...")
